...
SELECT 
    ...
    i.cantidad_minima,
    CASE 
        WHEN i.cantidad_disponible = 0 THEN 'Sin stock'
...
JOIN inventario i ON pd.id_producto_detalle = i.id_producto_detalle
...
SELECT 
    p.id_producto,
    p.nombre AS tipo_producto,
    COUNT(DISTINCT dv.id_venta) AS total_pedidos,
    SUM(dv.cantidad) AS unidades_vendidas,
    SUM(dv.subtotal) AS ingresos_totales,
    AVG(dv.precio_final) AS precio_promedio,
...
CREATE VIEW v_carritos_activos AS
SELECT 
    c.id_carrito,
    u.nombre AS usuario,
    u.email,
    COUNT(cd.id_carrito_detalle) AS items_en_carrito,
    SUM(cd.cantidad * cd.precio_unitario) AS valor_total,
    c.fecha_creacion,
    c.fecha_expiracion,
    EXTRACT(EPOCH FROM (c.fecha_expiracion - NOW()))/60 AS minutos_restantes
FROM carrito c
LEFT JOIN usuario u ON c.id_usuario = u.id_usuario
LEFT JOIN carrito_detalle cd ON c.id_carrito = cd.id_carrito
WHERE c.estado = 'activo'
GROUP BY c.id_carrito, u.nombre, u.email;
...
-- Aplicar a todas las tablas con fecha_actualizacion
CREATE TRIGGER trigger_actualizar_timestamp_usuario
...
CREATE TRIGGER trigger_actualizar_timestamp_producto_detalle
BEFORE UPDATE ON producto_detalle
FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();
...
CREATE TRIGGER trigger_actualizar_timestamp_carrito
...
-- Función: Actualizar valoración promedio de producto
...
EXECUTE FUNCTION actualizar_valoracion_producto();

-- ============================================================================

-- Función: Registrar movimiento de inventario al cambiar stock
...
BEGIN
    IF NEW.cantidad_disponible != OLD.cantidad_disponible THEN
        INSERT INTO movimiento_inventario (
            ...
            NEW.id_inventario,
            NEW.id_producto_detalle,
            CASE 
                WHEN NEW.cantidad_disponible > OLD.cantidad_disponible THEN 'entrada'::tipo_movimiento_inventario
                ELSE 'salida'::tipo_movimiento_inventario
            END,
...
-- Función: Actualizar stock al confirmar venta
...
BEGIN
    IF NEW.estado_pago = 'completado' AND OLD.estado_pago != 'completado' THEN
        ...
        UPDATE inventario i
        SET cantidad_disponible = cantidad_disponible - dv.cantidad,
            fecha_ultima_salida = CURRENT_TIMESTAMP
        FROM detalle_venta dv
        WHERE i.id_producto_detalle = dv.id_producto_detalle
          AND dv.id_venta = NEW.id_venta;
        
        -- Actualizar estadísticas de ventas
        UPDATE producto_detalle pd
        SET total_vendidos = total_vendidos + dv.cantidad,
            fecha_ultima_venta = CURRENT_TIMESTAMP
        FROM detalle_venta dv
        WHERE pd.id_producto_detalle = dv.id_producto_detalle
          AND dv.id_venta = NEW.id_venta;
    END IF;
...
-- Función: Validar stock antes de agregar al carrito
...
BEGIN
    ...
    FROM inventario
    WHERE id_producto_detalle = NEW.id_producto_detalle;
    
    IF stock_disponible < NEW.cantidad THEN
        RAISE EXCEPTION 'Stock insuficiente. Disponible: %', stock_disponible;
    END IF;
...
BEFORE INSERT OR UPDATE ON carrito_detalle
FOR EACH ROW
...
BEGIN
    IF NEW.numero_pedido IS NULL THEN
        NEW.numero_pedido := 'PED-' || 
                            TO_CHAR(CURRENT_TIMESTAMP, 'YYYYMMDD') || '-' ||
                            LPAD(NEW.id_venta::TEXT, 8, '0');
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION generar_slug()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER trigger_generar_slug_familia
BEFORE INSERT OR UPDATE ON familia
FOR EACH ROW
EXECUTE FUNCTION generar_slug();

CREATE TRIGGER trigger_generar_slug_categoria
...
CREATE TRIGGER trigger_generar_slug_producto_detalle
BEFORE INSERT OR UPDATE ON producto_detalle
FOR EACH ROW
EXECUTE FUNCTION generar_slug();
...
-- Insertar roles de administrador por defecto
INSERT INTO usuario (nombre, apellido, email, contrasena, rol, activo, email_verificado) VALUES
('Admin', 'Sistema', 'admin@tienda.com', '$argon2id$v=19$m=65536,t=3,p=4$salt$hash', 'super_admin', TRUE, TRUE);
...
CREATE INDEX idx_producto_detalle_busqueda ON producto_detalle 
USING gin(to_tsvector('spanish', nombre || ' ' || COALESCE(modelo, '') || ' ' || COALESCE(descripcion, '')));

-- Índice para reportes de ventas por fecha
CREATE INDEX idx_venta_fecha_estado ON venta(fecha_pedido, estado_pago) 
