...
<div class="min-h-[calc(100vh-4rem)] bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-slate-100">
	<div class="max-w-4xl mx-auto px-4 lg:px-0 py-8 space-y-6">
		...
		{:else if venta}
			<!-- Success Header -->
			<div class="rounded-3xl border border-green-500/40 bg-green-500/10 backdrop-blur-xl p-8 text-center space-y-3">
				<div class="flex justify-center mb-4">
					<svg class="w-16 h-16 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
					</svg>
				</div>
				<h1 class="text-3xl font-bold tracking-tight text-green-100">¡Pedido Confirmado!</h1>
						...
						<div class="rounded-2xl border border-white/10 bg-black/40 p-4 flex gap-4">
							...
							<div class="flex-1 min-w-0">
								...
								<div class="flex items-center gap-3 mt-2">
									<p class="text-xs text-slate-400">Cantidad: {item.cantidad}</p>
									{#if item.descuento_unitario > 0}
										<p class="text-xs text-green-400">Descuento: S/. {item.descuento_unitario.toFixed(2)}</p>
									{/if}
								</div>
							</div>
			...
			{#if venta.notas_cliente}
				<div class="rounded-3xl border border-white/10 bg-white/5 backdrop-blur-xl shadow-[0_18px_50px_rgba(0,0,0,0.45)] p-6 space-y-3">
					<h2 class="text-sm font-semibold uppercase tracking-wide text-slate-300">Observaciones</h2>
			...
			<div class="flex flex-col sm:flex-row gap-3">
				...
				<button
					type="button"
					class="flex-1 px-6 py-3 rounded-2xl text-sm font-semibold border border-white/10 bg-white/5 hover:bg-white/10 transition-colors"
